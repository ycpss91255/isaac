...
**Isaac Dev Kit**:
Robot-agnostic 的 4 層標準化開發框架 — L1 Model Pipeline（SW → URDF → USD）、L2 Asset Structure（geometry / material 分離）、L3 Sensor（YAML-driven camera / LiDAR / IMU）、L4 Control（IsaacDriver base class）。上層用 **Scene YAML** 宣告式組裝場景。
_Avoid_: simulation framework（太籠統，Isaac Sim 本身就是 simulation framework）、toolkit（跟 NVIDIA Isaac ROS toolkit 混淆）

**Robot**（在 isaac_ws 語境下）:
能主動移動的模型。走完整 `model/{sw,urdf,usd}/robot/<name>/` 三層 pipeline，需要 IsaacDriver subclass 控制。
_Avoid_: agent（在 RL 語境有不同含義）

**Environment Object**:
被動物件，需外力互動才會移動（push-back、pallet、shelf）。走 `model/{sw,urdf,usd}/object/<name>/` 三層 pipeline，不需要 driver。可有 joint（push-back 滑軌）但不主動驅動。
_Avoid_: static object（push-back 有 sliding joint，不是靜態的）、prop（VFX 術語，在 sim 語境不精確）

**Scene YAML**:
宣告式場景組裝設定（robot + objects + sensors + poses）。Runtime 動態組裝到 stage，不保留狀態。調整回各 model 源頭的 `.usda`。
_Avoid_: scene file（暗示持久儲存）、launch file（跟 ROS 2 launch 混淆）
...
- **Isaac Dev Kit** 的 4 層服務 Model A 和 Model B 兩軌 — 不同軌的差異在 **Scene YAML** 指向的 robot USD variant 和 driver subclass，框架本身不變
- **Robot** 與 **Environment Object** 的分類標準是「能否主動移動」，不是有沒有 joint（**Environment Object** 可以有 passive joint，如 push-back 滑軌）
